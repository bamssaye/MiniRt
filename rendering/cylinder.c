...
/*   Updated: 2025/02/12 06:45:34 by bamssaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color cylinder_checkerboard(t_cylinder cy, t_vec3d hit_point)
{
	double u = 0.5 + atan2(hit_point.z - cy.point.z, 
                          hit_point.x - cy.point.x) / (2 * PI);
    double v = fabs((hit_point.y - cy.point.y) / cy.len);
    double scale = 60;  // Adjust for checker size
    int x = floor(u * scale);
    int y = floor(v * scale);
    if ((x + y) % 2 == 0)
        return (t_color){0, 0, 0, 0};      // Black
    else
        return (t_color){255, 255, 255, 0}; // White
}
...
void	cy_inter(t_cylinder *cy, t_in_pa *f_inter)
{
	...
	if (check_cylinder_hit(cy, &tmp_inter[0]))
	{
		...
		f_inter->inters.shininess = 256.0;
		f_inter->closest = cpy_npc(tmp_inter[0].closest);
		f_inter->closest.color = cylinder_checkerboard(*cy, f_inter->closest.point);
		return ;
	}
