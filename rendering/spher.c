...
/*   Updated: 2025/02/09 19:48:13 by iel-koub         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_npc	c_sp_inter(t_ray ray, double dist, t_sphere sp)
{
	...
	return ((t_npc){.color = sp.color, .dista = dist, .normal = normal,
		.point = point});
}
...
t_color	spher_texture_color(t_tex tex, t_vec3d hit_point, t_sphere sp)
{
	...
	double(u), (v);
	int(x), (y);
	u = 0.5 + atan2(hit_point.z - sp.center.z, hit_point.x - sp.center.x) / (2
			* PI);
...
void	sp_inter(t_sphere *sp, t_in_pa *intersection)
{
	...
	if (dist > 0.0)
	{
		...
		if (sp->tex.img)
			intersection->closest.color = spher_texture_color(sp->tex,
