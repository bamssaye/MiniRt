...
/*   Updated: 2025/02/16 12:51:05 by bamssaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color pl_texture(t_tex *tex, t_vec3d *hpoint, t_pl *pl)
{
    ...
    double (u), (v);
    up = (t_vec3d){0, 1, 0, 0};
    right = vec3d_cross(&pl->normal, &up);
    ...
    corss = vec3d_cross(&right, &pl->normal);
    up_dir = vec3d_normalize(&corss);
    u = fmod(vec3d_dot(hpoint, &right) * 0.1, 1.0);
    v = fmod(vec3d_dot(hpoint, &up_dir) * 0.1, 1.0);
    if (u < 0)
        u += 1.0;
    if (v < 0)
        v += 1.0;
