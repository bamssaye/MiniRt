...
/*   Updated: 2025/02/12 07:02:11 by bamssaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	plane_inter(t_plane *plane, t_in_pa *param)
{
	...
	if (dist > 0)
	{
		...
		param->hit_clos = 1;
		param->inters.shininess = 0.2;
		param->closest.color = pl_checkborad(param->closest.point, *plane);
		// if (plane->tex.img)
...
//     if (dist > 0)
//     {
//         hit_point = vec3d_plus(param->ray->origin,vec3d_scale( dist, *ray));
//         t_vec3d right = {1, 0, 0, 0};
//         if (fabs(vec3d_dot(right, plane->normal)) > 0.9)
//             right = (t_vec3d){0, 1, 0,0};
//         t_vec3d forward = vec3d_normalize(vec3d_cross(right, plane->normal));
//         right = vec3d_normalize(vec3d_cross(forward, plane->normal));
//         t_vec3d local_pos = vec3d_minus(hit_point, plane->point);
//         double x = vec3d_dot(local_pos, right);
//         double z = vec3d_dot(local_pos, forward);
//         if (fabs(x) <= width/2 && fabs(z) <= length/2)
//         {
//             param->closest = pl_closest(*ray, *plane, dist, param->ray->origin);
//             param->hit_clos = 1;
//             param->inters.shininess = 0.2;
