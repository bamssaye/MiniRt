...
/*   Updated: 2025/02/12 03:16:12 by bamssaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
        ...
        if (tr.objt->type == LIGHT)
        {
            ...
            if (!tr.l_pa.stuck)
            {
                ...
                tr.l_pa.light.specular_color = color_scale(
                    fmin(0.5 * pow(fmax(0.0, vec3d_dot(reflect_dir, view_dir)), 32) , 1.0),
                    (t_color){180, 180, 180, 0});
