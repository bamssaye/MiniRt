	...
	t_inters_params *param, t_obslight *light_param)
{
	...
	copy_vec3d(&light_param->ray.direction, &light_param->light_direction);// sh
...
int color_a(t_color *re, t_color *a, t_color *b)
{
	re->r = (a->r * b->r) / 255;
	re->g = (a->g * b->g) / 255;
	re->b = (a->b * b->b) / 255;
...
void	trace_light_at_intersection(t_prog *prog, t_inters_params *param)
{
	...
	if (prog->has_light)
		...
		(l_param.angle_scale * prog->light.intensity ), &param->inters_color);
	color_a(&l_param.light.ambient_color, \
		&prog->ambient_color, &param->inters_color);
